...
sub download_urls {
    ...
    while ( scalar @urls ) {
        ...
        close URLS
            or
            die("apt-mirror: can't close intermediate file ($stage-urls.$i)");
...
foreach (@config_binaries) {
    ...
    if (@components) {
        ...
        foreach (@components) {
            add_url_to_download(
                $url . $_ . "/binary-" . $arch . "/Release" );
            add_url_to_download(
                $url . $_ . "/binary-" . $arch . "/Packages.gz" );
            add_url_to_download(
                $url . $_ . "/binary-" . $arch . "/Packages.bz2" );
        }
    }
    else {
        add_url_to_download( $uri . "/$distribution/Release" );
...
if ( get_variable("_autoclean") ) {
    ...
    chdir get_variable("mirror_path")
        or die("apt-mirror: can't chdir to mirror");
