...
sub process_directory {
  ...
  foreach ( grep { !/^\.$/ && !/^\.\.$/ } readdir($dir_h) ) {
    my $item = $dir . "/" . $_;
    $is_needed |= process_directory( $item, \%lsc ) if -d $item && !-l $item;
...
sub get_variable {
  ...
  while ( $value =~ s/\$(\w+)/$config_variables{$1}/xg ) {
    die("apt-mirror: too many substitution while evaluating variable")
      if ( $count-- ) < 0;
  }
  return $value;
}
...
sub download_urls {
  ...
  while ( scalar @urls ) {
    ...
    open URLS, ">" . get_variable("var_path") . "/$stage-urls.$i"
      or die("apt-mirror: can't write to intermediate file ($stage-urls.$i)");
    foreach (@part) { print URLS "$_\n"; }
    close URLS
      or die("apt-mirror: can't close intermediate file ($stage-urls.$i)");
    ...
    if ( $pid == 0 ) {
      exec 'wget', '--no-cache', '--limit-rate=' . get_variable("limit_rate"),
        '-t', '5', '-r', '-N', '-l', 'inf', '-o',
...
sub read_config {
  ...
  open CONFIG, "<$cf" or die("apt-mirror: can't open config file ($cf)");
  while (<CONFIG>) {
    next if /^\s*#/;
    next unless /\S/;
...
/deb-(alpha|amd64|armel|arm|hppa|hurd-i386|i386|ia64|lpia|m68k|mipsel|mips|powerpc|s390|sh|sparc)/
  ...
  die("Please explicitly specify 'defaultarch' in mirror.list")
...
sub proceed_index_gz {
  ...
  open STREAM, "<$path/$index"
    or die("apt-mirror: can't open index in proceed_index_gz");
...
foreach my $needed_directory (@needed_directories) {
  unless ( -d $needed_directory ) {
    mkdir($needed_directory)
      or cluck("apt-mirror: can't create $needed_directory dir. $!");
  }
}
...
open FILES_ALL, ">" . get_variable("var_path") . "/ALL"
  or die("apt-mirror: can't write to intermediate file (ALL)");
open FILES_NEW, ">" . get_variable("var_path") . "/NEW"
  or die("apt-mirror: can't write to intermediate file (NEW)");
open FILES_MD5, ">" . get_variable("var_path") . "/MD5"
  or die("apt-mirror: can't write to intermediate file (MD5)");
...
open CLEAN, ">" . get_variable("cleanscript")
  or die("apt-mirror: can't open clean script file");
